...
public class EmpConnector {
  ...
  /**
   * Disconnect the Bayeux Client from the server.
   * Normally should be used in case of an exception being raised or other
   * situation that would necessitate a reconnection.
   */
  ...
  public void stop() {
    ...
    if (httpClient != null) {
      try {
        log.info("Stopping the HTTP Client in EmpConnector");
        httpClient.stop();
      } catch (Exception e) {
